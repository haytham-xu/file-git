...
class LoggerManager:
    ...
    def log_success(self, action, local_path, remote_path):
        if not file_support.is_exist(self.log_success_file):
            file_support.create_file(self.log_success_file)
    ...
    def log_error(self, action, local_path, remote_path, error_track):
        if not file_support.is_exist(self.log_error_file):
            file_support.create_file(self.log_error_file)
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M")

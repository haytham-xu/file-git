...
class FilegitConfig():
    ...
    def __init__(self, mode=None, password=None, local_path=None, remote_path=None, app_id=None,
        ...
        self.local_vpath = local_path
        self.remote_vpath = remote_path
        self.app_id = app_id
    ...
    def init(self, config_file_vpath):
        if not file_support.is_local_exist(config_file_vpath):
            raise FileNotFoundError(f"config_instance file '{config_file_vpath}' does not exist.")
        config_data = file_support.real_read_json_file(config_file_vpath)
        self.mode = Mode.from_string(config_data.get('mode', self.mode))
        self.password = config_data.get('password', self.password)
        self.local_vpath = config_data.get('local_path', self.local_vpath)
        self.remote_vpath = config_data.get('remote_path', self.remote_vpath)
    ...
    def write_config(self, config_file_vpath):
        config_data = {
            'mode': self.mode.value,
            'password': self.password,
            'local_path': self.local_vpath,
            'remote_path': self.remote_vpath,
            'app_id': self.app_id,
        ...
        file_support.real_write_json_file(config_file_vpath, config_data)
    ...
    def get_local_vpath(self):
        return self.local_vpath
    def set_local_vpath(self, local_path):
        self.local_vpath = local_path

    def get_remote_vpath(self):
        return self.remote_vpath
    def set_remote_vpath(self, remote_path):
        self.remote_vpath = remote_path
